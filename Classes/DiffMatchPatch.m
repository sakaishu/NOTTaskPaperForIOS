...
+ (id)diffWithOperationType:(Operation)operation text:(NSString *)text {
    return  [[[Diff alloc] initWithOperation:operation andText:text] autorelease];
}

+ (id)equal:(NSString *)text {
	return [self diffWithOperationType:DIFF_EQUAL text:text];
}

+ (id)insert:(NSString *)text {
	return [self diffWithOperationType:DIFF_INSERT text:text];
}

+ (id)delete:(NSString *)text {
	return [self diffWithOperationType:DIFF_DELETE text:text];
}
...
- (Patch *)reverse {
    Patch *reverse = [[[Patch alloc] init] autorelease];
    reverse.start1 = start2;
    reverse.start2 = start1;
    reverse.length1 = length2;
    reverse.length2 = length1;
    
    for (Diff *each in diffs) {
        switch (each.operation) {
            case DIFF_INSERT:
                [reverse.diffs addObject:[Diff delete:each.text]];
                break;
            case DIFF_DELETE:
                [reverse.diffs addObject:[Diff insert:each.text]];
                break;
            default:
                [reverse.diffs addObject:[Diff equal:each.text]];
                break;
        }
    }
    return reverse;
...
- (void)delegate:(id)delegate selector:(SEL)selector replaceCharactersInRange:(NSRange)aRange withString:(NSString *)aString padding:(NSString *)padding {
	if (delegate == nil || selector == NULL) {
		return;
	}
    
	for (NSInteger i = 0; i < [padding length]; i++) {
		unichar p = [padding characterAtIndex:i];
		if ([aString length] > 0) {
			if ([aString characterAtIndex:0] == p) {
				aRange.location++;
				aRange.length--;
	...
	aRange.location -= [padding length];
    
	for (NSInteger i = [padding length] - 1; i >= 0; i--) {
		unichar p = [padding characterAtIndex:i];
		if ([aString length] > 0) {
			if ([aString characterAtIndex:[aString length] - 1] == p) {
				aRange.length--;
	...
	[invocation setArgument:&aString atIndex:3];
    ...
    for (Patch *aPatch in patches) {
        ...
        if (start_loc == NSNotFound) {
            ...
            if (text1 == text2) {
                ...
                NSString *diffText2 = [self diff_text2:aPatch.diffs];
                [self delegate:delegate selector:selector replaceCharactersInRange:NSMakeRange(start_loc, [text1 length]) withString:diffText2 padding:nullPadding];
                [textMutable replaceCharactersInRange:NSMakeRange(start_loc, text1.length) withString:diffText2];
            } else {
                ...
                if (text1.length > Match_MaxBits
                    ...
                    for (Diff *aDiff in aPatch.diffs) {
                        if (aDiff.operation != DIFF_EQUAL) {
                            ...
                            if (aDiff.operation == DIFF_INSERT) {
                                // Insertion
                                [self delegate:delegate selector:selector replaceCharactersInRange:NSMakeRange(start_loc + index2, 0) withString:aDiff.text padding:nullPadding];
                                [textMutable insertString:aDiff.text atIndex:(start_loc + index2)];
                            } else if (aDiff.operation == DIFF_DELETE) {
                                // Deletion
                                NSInteger start = start_loc + index2;
								NSInteger diff = [self diff_xIndexIn:diffs location:index1 + [aDiff.text length]];
								NSRange range = NSMakeRange(start, (start_loc + diff) - start);
								[self delegate:delegate selector:selector replaceCharactersInRange:range withString:@"" padding:nullPadding];
...
- (NSArray *)patch_apply:(NSArray *)sourcePatches
    ...
    return [self patch_apply:sourcePatches toString:text delegate:nil selector:NULL];
}
